...
Download [`ects-tracker-standalone.html`](./ects-tracker-standalone.html) and open it directly in a browser, or host it anywhere that serves static files (GitHub Pages, Netlify, Vercel, or just your own web server). It's a single HTML file with everything bundled in, including React itself, so there's no build step and no dependencies to install.

### 2. Use it as a Claude artifact
[`ects-tracker.jsx`](./ects-tracker.jsx) is the same app written as a React component, meant to be pasted into [Claude](https://claude.ai) as an artifact. This is the source of truth; the standalone HTML is generated from it.
